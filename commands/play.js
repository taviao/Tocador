...
module.exports = {
  name: "play",
  description: "vamo escutar um musicao!!",
  ...
  run: async (client, interaction) => {
    try {
      ...
      if (!name) return interaction.reply({ content: `❌ Coloca a música certa burro.`, ephemeral: true }).catch(e => { });
      let res;
      try {
        ...
        return interaction.editReply({ content: `❌ achei nada não` }).catch(e => { });
      }

      if (!res || !res.length || !res.length > 1) return interaction.reply({ content: `❌ Sem resultado`, ephemeral: true }).catch(e => { });
          ...
          switch (button.customId) {
            ...
            default: {
              ...
              try {
                ...
                await interaction.editReply({ content: `❌ Sem resultado!`, ephemeral: true }).catch(e => { });
              }
              return collector.stop();
            }
          }
