...
module.exports = {
  name: "loop",
  description: "Ativa ou desativa o modo loop.",
  ...
  run: async (client, interaction) => {
    ...
    try {
      ...
      if (!queue || !queue.playing) return interaction.reply({ content: '⚠️ Sem música tocando!!', ephemeral: true }).catch(e => { })
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription('**Loop ativado! Deixe a música tocar sem parar. **')
      ...
      interaction?.reply({ embeds: [embed], components: [button], fetchReply: true }).then(async Message => {
        ...
        col.on('collect', async (button) => {
          ...
          if (!queue1 || !queue1.playing) {
            await interaction?.editReply({ content: '⚠️ Sem música tocando!!', ephemeral: true }).catch(e => { })
          ...
          switch (button.customId) {
            ...
            case 'close':
              if (queue.repeatMode === 0) {
                await button?.deferUpdate().catch(e => {})
                return interaction?.editReply({ content: '⚠️ Looping Off!!', ephemeral: true }).catch(e => { })
              }
