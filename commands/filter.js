...
module.exports = {
  name: "filter",
  description: "Adiciona filtro de áudio à música.",
...
if(!filters?.includes(button?.customId)) return
      ...
      if (!filtre) return interaction?.editReply({ content: '❌ Nome Invalido', ephemeral: true }).catch(e => { })
     filtre = filtre?.toLowerCase()
   
      if (filters?.includes(filtre?.toLowerCase())) {
        if (queue?.filters?.has(filtre)) {
          queue?.filters?.remove(filtre)
          embed?.setDescription(`Magic : **{filter}**, Status Aplicados: **{status}**`.replace("{filter}", filtre).replace("{status}", "❌"))
