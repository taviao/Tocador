...
return interaction?.reply({ content: "Rate Limited.", ephemeral: true })
    ...
    function cmd_loader() {
        if (interaction?.type === InteractionType.ApplicationCommand) {
            ...
            if (!interaction?.member?.voice?.channelId) return interaction?.reply({ content: `🔴 TEM QUE ENTRAR EM UM CANAL DE VOZ BURRRRRO!!`, ephemeral: true }).catch(e => { })
            ...
            return interaction?.reply({ content: `🔴 Tem que estar no mesmo canal de voz, burro!!`, ephemeral: true }).catch(e => { })
            }
            }
            }
            ...
            return interaction?.reply({ content: `▶️ Permissões Ausentes: **${props?.permissions?.replace("0x0000000000000020", "MANAGE GUILD")?.replace("0x0000000000000800", "ENVIE MENSAGENS") || "ENVIEM MENSAGENS"}**`, ephemeral: true });
            }
            } catch (e) {
